...
with open('src/employees.json') as json_file:
    ...
    employees_3_years_of_experience = list(filter(
            lambda emp: lesser_than(emp, 'years_of_experience', 3),
            data
        ))

    print(f'Employees with less than 3 years of experience:{len(employees_3_years_of_experience)}')
    ...
    print(f'of experience and less than 25 years:{len(employees_3_years_of_experience_and_less_than_25_years)}')

    # 3. let's get all employees that don't have a shirt size :(

    employees_with_no_shirt_size = list(
        filter(
            lambda emp: not emp['shirt_size'],
            data
        )
    )
